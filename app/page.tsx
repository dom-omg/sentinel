...
function IntroAnimation({ onComplete }: { onComplete: () => void }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ position:'absolute', width:500, height:500, background:'radial-gradient(circle,rgba(63,185,80,0.07) 0%,transparent 70%)', borderRadius:'50%', pointerEvents:'none' }}/>
      ...
      <p style={{
        color:'#3fb950', fontSize:11, fontWeight:500, letterSpacing:'0.18em',
      ...
      <div style={{ display:'flex', gap:18, alignItems:'flex-end', height:52 }}>
        {soldiersIn.map((isIn, i) => (
          <div key={i} style={{
            ...
            filter: 'drop-shadow(0 0 7px rgba(63,185,80,0.55))',
          }}>
            <ShieldSoldier color={i === 3 ? '#9dffb2' : '#3fb950'} delay={i * 170} />
          </div>
        ))}
      </div>

      <div style={{ position:'absolute', bottom:40, left:'50%', transform:'translateX(-50%)', width:200, height:2, background:'rgba(63,185,80,0.12)', borderRadius:2, overflow:'hidden' }}>
        <div style={{
          height:'100%',
